  ...
  return (
    <div
      className={`group relative min-h-[24px] flex items-start rounded transition-all duration-150 ease-in-out
        ${isSelected ? 'bg-[#f0f2fe] dark:bg-blue-900/30 ring-1 ring-blue-200 dark:ring-blue-700 shadow-sm' : 'hover:bg-[#f5f5f6] dark:hover:bg-gray-700'}`}
    ...
    >
      ...
      <div className="relative flex-1 py-0.5 px-1">
        ...
        {showCommands && (
          <div
            ref={commandsRef}
            className="absolute left-0 top-full mt-1 w-64 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-[#e7e7e9] dark:border-gray-700 py-2 z-50
          ...
          >
            {filteredCommands.map((cmd, index) => (
              <button
                key={cmd.id}
                className={`
                  w-full text-left px-3 py-2 flex items-center space-x-3 hover:bg-[#f5f5f6] dark:hover:bg-gray-700
                  ${index === selectedCommandIndex ? 'bg-[#f5f5f6] dark:bg-gray-700' : ''}
                `}
